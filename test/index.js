...
describe('nodebb-plugin-category-queue', () => {
	...
	it('should not queue any new topics by default', async () => {
		assert(
			...
			"Unprivileged user's topic was queued",
		);
		assert(
			!await posts.shouldQueue(exemptUid, { cid, content: 'Just a test topic' }),
			"Exempt user's topic was queued",
		);
		assert(
	...
	it('should not queue any new replies by default', async () => {
		assert(
			...
			"Unprivileged user's reply was queued",
		);
		assert(
			!await posts.shouldQueue(exemptUid, { tid, content: 'Just a test reply' }),
			"Exempt user's reply was queued",
		);
		assert(
	...
	it('should queue new topics by all users if enabled', async () => {
		...
		assert(
			await posts.shouldQueue(adminUid, { cid, content: 'Just a test topic' }),
			"Administrator's topic was not queued",
		);
	...
	it('should queue new replies by all users if enabled', async () => {
		...
		assert(
			await posts.shouldQueue(adminUid, { tid, content: 'Just a test reply' }),
			"Administrator's reply was not queued",
		);
		await meta.settings.setOne('category-queue', `${cid}-enabled`, 'off');
	});
